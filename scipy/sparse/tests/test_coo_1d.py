...
class _TestCommon1D:
    ...
    This testing frameworks assumes the format passes the `test_base.py` 2d tests.
    """
    @classmethod
    def init_class(cls):
        # Canonical data.
    ...
    def test_expm(self):
        M = array([1, 0, 2], float)
        sM = self.spcreator(M, shape=(3,), dtype=float)
        Mexp = scipy.linalg.expm(M)

        N = array([3., 0., 1.])
        sN = self.spcreator(N, shape=(3,), dtype=float)
        ...
        assert_array_almost_equal((sMexp - Mexp), zeros(3))
        assert_array_almost_equal((sNexp - Nexp), zeros(3))
